...
SNPS_EVGraphRuntime::SNPS_EVGraphRuntime(const std::string& graph_json, DSOModule* module) {
  assert(module);
  module_ = module;
  picojson::value v;
...
SNPS_EVGraphRuntime::~SNPS_EVGraphRuntime() {}

void SNPS_EVGraphRuntime::Run() {
  for (size_t i = 0; i < op_execs_.size(); ++i) {
    if (op_execs_[i]) op_execs_[i]();
  }
}

void SNPS_EVGraphRuntime::SetInput(int index, DLTensor* data_in) {
  assert(static_cast<size_t>(index) < input_nodes_.size());
  uint32_t eid = this->entry_id(input_nodes_[index], 0);
  data_entry_[eid].CopyFrom(data_in);
}

void SNPS_EVGraphRuntime::CopyOutputTo(int index, DLTensor* data_out) {
...
void SNPS_EVGraphRuntime::SetupStorage() {
...
void SNPS_EVGraphRuntime::SetupOpExecs() {
