...
struct TVMSNPS_EVSessionThreadLocalEntry {
  std::stack<ObjectPtr<SNPS_EVSession>> session_stack;
};

typedef dmlc::ThreadLocalStore<TVMSNPS_EVSessionThreadLocalEntry> TVMSNPS_EVSessionThreadLocalStore;

ObjectPtr<SNPS_EVSession>& SNPS_EVSession::Current() {
  TVMSNPS_EVSessionThreadLocalEntry *entry = TVMSNPS_EVSessionThreadLocalStore::Get();
  CHECK_GT(entry->session_stack.size(), 0) << "No current session";
  return entry->session_stack.top();
}

void SNPS_EVSession::EnterWithScope(ObjectPtr<SNPS_EVSession> session) {
  TVMSNPS_EVSessionThreadLocalEntry *entry = TVMSNPS_EVSessionThreadLocalStore::Get();
  entry->session_stack.push(session);
}

void SNPS_EVSession::ExitWithScope() {
  TVMSNPS_EVSessionThreadLocalEntry *entry = TVMSNPS_EVSessionThreadLocalStore::Get();
  CHECK(!entry->session_stack.empty());
  entry->session_stack.pop();
}

SNPS_EVSession::SNPS_EVSession(
  ...
  if (comms_method == "host") {
    ...
    section_allocators_[0] = std::make_shared<SNPS_EVSectionAllocator>(DevMemRegion {
      .start = curr_addr,
      .size = text_size,
    }, word_size_);
    curr_addr += text_size;
    section_allocators_[1] = std::make_shared<SNPS_EVSectionAllocator>(DevMemRegion {
      .start = curr_addr,
      .size = rodata_size,
    }, word_size_);
    curr_addr += rodata_size;
    section_allocators_[2] = std::make_shared<SNPS_EVSectionAllocator>(DevMemRegion {
      .start = curr_addr,
      .size = data_size,
    }, word_size_);
    curr_addr += data_size;
    section_allocators_[3] = std::make_shared<SNPS_EVSectionAllocator>(DevMemRegion {
      .start = curr_addr,
      .size = bss_size,
    }, word_size_);
    curr_addr += bss_size;
    section_allocators_[4] = std::make_shared<SNPS_EVSectionAllocator>(DevMemRegion {
      .start = curr_addr,
      .size = args_size,
    }, word_size_);
    curr_addr += args_size;
    section_allocators_[5] = std::make_shared<SNPS_EVSectionAllocator>(DevMemRegion {
      .start = curr_addr,
      .size = heap_size,
    }, word_size_);
    curr_addr += heap_size;
    section_allocators_[6] = std::make_shared<SNPS_EVSectionAllocator>(DevMemRegion {
      .start = curr_addr,
      .size = workspace_size,
    }, word_size_);
    curr_addr += workspace_size;
    section_allocators_[7] = std::make_shared<SNPS_EVSectionAllocator>(DevMemRegion {
      .start = curr_addr,
      .size = stack_size,
    }, word_size_);
    ...
    LOG(FATAL) << "unsupported snps_ev low-level device";
  }
  ...
  std::shared_ptr<SNPS_EVSectionAllocator> ws_allocator = GetAllocator(SectionKind::kWorkspace);
...
double SNPS_EVSession::PushToExecQueue(DevPtr func_ptr, const TVMArgs& args) {
  if (thumb_mode_) {
    func_ptr += 1;
  }
...
BinaryInfo SNPS_EVSession::LoadBinary(const std::string& binary_path, bool patch_dylib_pointers) {
...
std::tuple<DevPtr, DevPtr> SNPS_EVSession::EncoderAppend(
  ...
  for (int i = 0; i < num_args; i++) {
    switch (type_codes[i]) {
      ...
      case kArrayHandle: {
        ...
        // All uTVM arrays store a `SNPS_EVDevSpace` struct in their `data` field,
        ...
        base_arr_handle->data = reinterpret_cast<SNPS_EVDevSpace*>(old_data)->data;
...
template <typename T>
DevPtr SNPS_EVSession::EncoderAppend(TargetDataLayoutEncoder* encoder, const TVMArray& arr) {
  ...
  CHECK(dev_arr.ctx.device_type == static_cast<DLDeviceType>(kDLSNPS_EV))
    << "attempt to write TVMArray with non-snps_ev device type";
  // Update the device type to CPU, because from the snps_evcontroller's
  // perspective, it is.
...
void SNPS_EVSession::CheckDeviceError() {
  ...
  if (return_code) {
    ...
    LOG(FATAL) << "error during snps_ev function execution:\n"
...
void SNPS_EVSession::PatchImplHole(const SymbolMap& symbol_map, const std::string& func_name) {
...
std::string SNPS_EVSession::ReadString(DevPtr str_addr) {
...
DevPtr SNPS_EVSession::AllocateInSection(SectionKind type, size_t size) {
  return GetAllocator(type)->Allocate(size);
}

void SNPS_EVSession::FreeInSection(SectionKind type, DevPtr addr) {
  return GetAllocator(type)->Free(addr);
}

template <typename T>
T SNPS_EVSession::DevSymbolRead(const SymbolMap& symbol_map, const std::string& symbol) {
...
template <typename T>
void SNPS_EVSession::DevSymbolWrite(const SymbolMap& symbol_map,
...
PackedFunc SNPS_EVSession::GetFunction(
    const std::string& name,
    const ObjectPtr<Object>& sptr_to_self) {
  if (name == "enter") {
    return PackedFunc([sptr_to_self, this](TVMArgs args, TVMRetValue* rv) {
      SNPS_EVSession::EnterWithScope(GetObjectPtr<SNPS_EVSession>(this));
    });
  } else if (name == "exit") {
    return PackedFunc([sptr_to_self](TVMArgs args, TVMRetValue* rv) {
      SNPS_EVSession::ExitWithScope();
    });
  } else {
    return PackedFunc();
  }
...
// create snps_ev session and low-level device from Python frontend
TVM_REGISTER_GLOBAL("snps_ev._CreateSession")
.set_body([](TVMArgs args, TVMRetValue* rv) {
    ...
    int port = args[22];
    ObjectPtr<SNPS_EVSession> session = make_object<SNPS_EVSession>(
        comms_method,
        binary_path,
        toolchain_prefix,
        text_start,
