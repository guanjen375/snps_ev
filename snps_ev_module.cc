...
namespace runtime {
/*!
 * \brief module for uTVM snps_ev devices
 */
class SNPS_EVModuleNode final : public ModuleNode {
 public:
  SNPS_EVModuleNode() {}
   ...
   */
  void InitSNPS_EVModule(const std::string& binary_path) {
    session_ = SNPS_EVSession::Current();
 ...
 private:
  ...
  ObjectPtr<SNPS_EVSession> session_;
};

class SNPS_EVWrappedFunc {
 public:
  SNPS_EVWrappedFunc(ObjectPtr<SNPS_EVSession> session,
                   DevPtr func_ptr) {
    session_ = session;
    func_ptr_ = func_ptr;
  }
...
PackedFunc SNPS_EVModuleNode::GetFunction(
  ...
  SNPS_EVWrappedFunc f(session_, func_ptr);
  return PackedFunc(f);
}

// register loadfile function to load module from Python frontend
TVM_REGISTER_GLOBAL("module.loadfile_snps_ev_dev")
.set_body([](TVMArgs args, TVMRetValue* rv) {
    auto n = make_object<SNPS_EVModuleNode>();
    n->InitSNPS_EVModule(args[0]);
    *rv = runtime::Module(n);
  });
}  // namespace runtime
}  // namespace tvm
